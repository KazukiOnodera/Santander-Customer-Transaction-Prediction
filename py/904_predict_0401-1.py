...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  1 23:07:12 2019
...
SUBMIT_FILE_PATH = '../output/0401-1.csv.gz'
...
sub_fake = sub.iloc[fake_index]
sub_real = sub.drop(fake_index).reset_index(drop=True)

for model in tqdm(model_all):
    sub_real['target'] += pd.Series(model.predict(X_test)).rank()
sub_real['target'] /= sub_real['target'].max()

sub = pd.concat([sub_fake, sub_real]).sort_values('ID_code').reset_index(drop=True)
